...
double DoubleExponential::Integrate(const double &x1, const double &x2,
    ...
    for(size_t k = 1; k <= _phases; ++k) {
        ...
        DLOG_F(3, "Iteration %zu (value, error): %e, %e", k, s1, err);
        DLOG_F(3, "Number of evaluations = %zu", evals);

        if(k > 3 && err < eps1) {
            DLOG_F(INFO, "Result (value, error): %e, %e", s1, err);
            return s1;
        }
        if(k >= 3 && err < eps2) {
            DLOG_F(INFO, "Result (value, error): %e, %e", s1, err);
    ...
    DLOG_F(3, "Failed to converge");
    DLOG_F(3, "Result (value, error): %e, %e", s1, err);
    return s1;
}
