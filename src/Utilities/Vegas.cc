...
VegasResult::VegasResult(bool weighted_) : weighted(weighted_) {
    mean = mean2 = var = var2 = 0;
    meanDivVar = mean2DivVar = OneDivVar = 0;
    sum = sum2 = sumVar = 0;
    n = 0;
}
...
Vegas::Vegas(const Utility::AdaptiveMap& map_, const IO::Settings& settings) : map(map_) {
    nstrats = 0;
    sumSigF = lim::max();
    lastNEval = 0;
    
    Set(settings);
    size_t seed = settings.GetSetting<size_t>("iSeed");
#if USING_MPI
    seed += ResBos::mpi -> Rank();
#endif
    rand = new Random(seed);
}

void Vegas::Set(const IO::Settings& settings) {
    neval = settings.GetSetting<size_t>("evaluations");
    maxinc = settings.GetSetting<size_t>("maxinc");
    nCubeBatch = settings.GetSetting<size_t>("nCubeBatch");
    maxCube = settings.GetSetting<size_t>("maxCube");
    maxEvalCube = settings.GetSetting<size_t>("maxEvalCube");
    nitn = settings.GetSetting<size_t>("iterations");
    alpha = settings.GetSetting<double>("alpha");
    beta = settings.GetSetting<double>("beta");
    adapt = settings.GetSetting<bool>("adapt");
    adaptErrors = settings.GetSetting<bool>("adaptErrors");
    rtol = settings.GetSetting<double>("vegas_rtol");
    atol = settings.GetSetting<double>("vegas_atol");
    sync = settings.GetSetting<bool>("sync");
...
void Vegas::Setup() {
    ndims = map.GetDims();
    double nevalD = static_cast<double>(neval);
    double ndimsD = static_cast<double>(ndims);
    double nEvalEff = beta > 0 ? (nevalD / 2.0) : nevalD;
    size_t ns = static_cast<size_t>(pow(nEvalEff/2.0,1.0/ndimsD)); // Stratifications / axis
    size_t ni = static_cast<size_t>(nevalD / 10.0);                // Increments / axis
...
void Vegas::RandomBatch(size_t cubeBase, size_t nCubeBatch_, 
    ...
    Batch2D yran = rand->Get(nEvalBatch, ndims);
    size_t iStart = 0;
    for(size_t iCube = 0; iCube < nCubeBatch_; ++iCube) {
...
Batch Vegas::MakeBatch(const Func& fcn, Batch2D x_, Batch wgt) {
    Batch res;
    for(size_t i = 0; i < x_.size(); ++i) res.push_back(fcn(x_[i],wgt[i]));
    return res;
}

VegasPt Vegas::operator() (const Func& fcn) {
    ...
    for(size_t itn = 0; itn < nitn; ++itn) {
        ...
        for(size_t cubeBase = 0; cubeBase < nCube; cubeBase += nCubeBatch_) {
            ...
            for(auto iCube : cube) {
                ...
                if(beta > 0 && adapt) sumSigF_ += pow(sigF2, beta/2.0);
...
void KBNSummation::AddTerm(double value) noexcept { ///Function to add a value to the sum that is being calculated and keep the correction term
    double t = sum + value;
